...
#include "mpu6050/kalman_filter.h"
#include <math.h>
#include "esp_log.h"
...
#define ACCELEROMETER_READING_FREQUENCY_MS	     REFRESH_RATE * 10	// 5ms second
...
uint8_t calibration_mode = CALIBRATION_MODE_SIMPLE;
...
void vMPU6050Task( void *pvParameters )
{
    ...
    if (uMPU6050Init() != MPU6050_SUCESS) {
        vTaskDelete(NULL);
        ESP_LOGI("HEY","TASK_DELETED");
        return;
    } else {ESP_LOGI("HEY","IT's OK HERE");}
